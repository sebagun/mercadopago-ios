...
- (NSDictionary *)fingerPrint{
    ...
    [dictionary setObject:device.totalDiskSpace forKey:@"disk_space"];
    [dictionary setObject:device.freeDiskSpace forKey:@"free_disk_space"];
    ...
    if (![self isNullOrEmpty:device.model]) {
    ...
    if (![self isNullOrEmpty:device.platform]) {
    ...
    if (![self isNullOrEmpty:device.name]) {
